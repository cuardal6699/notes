...
const Home = ({notes}: Notes) => {
 ...
 const handleSubmbit = async (data: FormData) => {
  try {
    create(data)
  } catch (error) {
    console.log(error);
  ...
  return (
    <div>
      <h1 className='text-center font-bold text-2xl mt-4 pb-2'>Notes</h1>
      <form onSubmit={e=>{
        e.preventDefault()
        handleSubmbit(form)
          ...
          {notes.map(note => (
            <li key={note.id} className='border-b border-gray-600 p-2'>
              <div className='flex justify-between'>
                ...
                <button onClick={()=> deleteNote(note.id)} className='bg-red-500 px-3 ml-2 text-white rounded'>X</button>
              </div>
            </li>
          ))}
        </ul>
